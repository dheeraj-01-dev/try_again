...
export const loginUser = async (req :any, res :any) => {
  ...
  try {
    ...
    if(!user){return res.status(404).json({success: false, message: "please sign up first."})};


    const { _id, name, ffUid, userName, createAt } = user;   

    const passMatch = await bcrypt.compare( password, user.password);
    if(!passMatch){return res.status(404).json({success: false, message: "password doesn't matched!"})}
...
export const getSingleUser =async (req :any, res :any) => {
  ...
  if(!user || !authToken){return res.status(404).json({success: false, message: "please enter all required fields." })}
  try {
    ...
    if(verifiedToken._id!==user){return res.status(401).json({success: false, message: "unauthorized...." })}
...
export const resetPass = async (req :any, res :any) => {
  ...
  if(!phone &&!email ||!newPassword ){return res.status(404).json({success: false, message: "please provide all fields."})};
  ...
  try {
    ...
    if(updatedUser.matchedCount!==1){return res.status(404).json({success: false, message: "please sign up first."})}
    if(updatedUser.modifiedCount!==1){return res.status(500).json({success: false, message: "please try again."})}
...
export const updateUserData = async (req  :any, res :any) => {
  ...
  if(!newName &&!newUserName &&!newffUid){return res.status(422).json({success: false, message: "please provide atleast one field."})}
  try {
    const verifiedToken :any = jwt.verify(authToken, jwt_secret);
    const verifiedUser = await userModel.findById(verifiedToken._id);
    
    if(!verifiedUser || user!==verifiedUser?._id.toHexString()){return res.status(404).json({success: false, message: "invalid user"})};
    ...
    if(updatedUser.modifiedCount!==1){return res.status(203).json({success: false, message: "rest to default..."})};
